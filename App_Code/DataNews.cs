...
public class DataNews : DataClass
{
    ...
    public int getDataCount(int group = 0, bool NoiBat = false, String seach = "",bool selecthiden = false)
    {
        try
        {
            SqlCommand Cmd = this.getSQLConnect();
            Cmd.CommandText = "SELECT TOP 1 COUNT(*) FROM tblNews AS P LEFT JOIN tblNewsGroup AS G ON P.CatId = G.ID WHERE P.NSTATUS != 2";

            if (selecthiden == false)
            {
                Cmd.CommandText += " AND G.NSTATUS = 1";
            }
    ...
    public DataTable getDataTop(int limit = 0, int group = 0, int page = 1, bool NoiBat = false, String seach = "",String sapXep = "DESC",bool selecthiden = false)
    {
        try
        {
            String top = "";
